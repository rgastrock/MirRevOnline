...
There are currently two data sets (currently analyzing up to September 2020). Note that these experiments are ongoing, so the data sets should be updated.
1) Summer - this includes the instructed participants
2) Fall - this includes the non-instructed participants

## Cleaning the data

Some participants had to be removed due to different issues, including:
1) computer crashing/technical difficulties with the experiment
2) data is missing/ not saved appropriately
3) did not use a mouse or trackpad
4) cleaned baseline (aligned) reaches
...
There seems to be a difference between target locations during the aligned session, although this difference is likely due to reach biases in relation to each target location. Regardless, reaches during the aligned session are near the zero degree mark.
...
To get a complete picture of how participants are performing, we can plot the distribution of angular reach deviations of all participants within each group. The plots below will show a circular density distribution, where zero degrees corresponds to target locations, and perfect compensation is marked accordingly, depending on the target location. The plots also show how the distribution of responses changes across the 90 trials of mirror reversed training. Here, I will show distributions for trials 1, 2, and 90.

## Non - instructed group

```{r}
plotGroupCircFreq(set='fa2020')
```

For both targets, we see a bimodal distribution in trial 1. We also see a general shift of responses towards the correct reach direction (i.e. highest peak in distribution moves towards perfect compensation location for each target).

### Move throughs or exploration

To investigate why we get a subset of participants already reaching in the correct direction relative to the mirror axis by trial 1, our hypothesis is that participants are making exploratory movements. That is, before the first mirror reversed trial begins, participants had to bring the cursor to the start position at the centre of the screen. However, during this time, the mirror reversal has already been implemented, which could have led to participants knowing which approximate direction they should reach, in order to successfully compelte the upcoming trial. 
...
We see that move throughs are very much persistent on trial 1. 30 out of 54 participants have move throughs in trial 1 for the 30 degree target, and this is 28 out of 52 participants for the 60 degree target. Therefore, we can plot the circular density distributions for trial 1 while separating participants into subgroups of those with or without move throughs (or exploration).
...
For the 30 degree target, we find a significant relationship between step 1 movement time and the corresponding reach deviation for that trial.

```{r}
plotGroupRDMT(group='60', set='fa2020')
```

For the 60 degree target, we find the same results. Note that one participant (216814) has been removed from this test for having an outlier movement time for this trial.
...
Similar to the movement time analysis, path length is another variable that can support our findings regarding exploratory behavior. Path length is essentially the path/trajectory that participants take from one point to another. For example, shorter path lengths suggest straight and smooth movements to the target.

```{r}
plotGroupRDPL(group='30', set='fa2020')
```

For the 30 degree target, we do not find a significant relationship between step 1 path length and the corresponding reach deviation for that trial.

```{r}
plotGroupRDPL(group='60', set='fa2020')
```

For the 60 degree target, we find a significant relationship. Note that one participant (216814) has been removed from this test for having an outlier movement time for this trial.
...
We find that path lengths are initially longer during the first few trials of the mirror reversal, but eventually get close to baseline levels.The variability also becomes reduced as mirror reversal training progresses.
...
```{r}
plotGroupCircFreq(set='su2020')
```

Both targets show distributed responses for trial 1. We also see a general shift of responses towards the correct reach direction (i.e. highest peak in distribution moves towards perfect compensation location for each target).
...
We see that move throughs are only persistent on trial 1 for the 30 degree target, given that it is always the first trial for this group. 56 out of 109 participants have move throughs in trial 1 for the 30 degree target. Therefore, we can plot the circular density distribution for trial 1 of this target location, while separating participants into subgroups of those with or without move throughs (or exploration).
...
For the 30 degree target, we find a significant relationship between step 1 movement time and the corresponding reach deviation for that trial. Note that for this group, it would be unnecessary to analyze the 60 degree target, as it always occurs as the second trial, such that participants have had enough opportunities to explore the workspace by then.
...
When we separate participants into the two subgroups of explorers and non-explorers, we find similar results as those in movement time.
...
## Non-instructed group
Current sample size: Mouse (N = 159), Trackpad (N = 360)
...
## Instructed group
Current sample size: Mouse (N = 28), Trackpad (N = 77)
...
## Non-instructed group
Current sample size: Male (N = 133), Female (N = 385)
...
## Instructed group
Current sample size: Male (N = 27), Female (N = 78)
...
It is viable to investigate the mirror reversal task on an online platform. However, participants exploring the workspace speeds up learning of the perturbation. Regardless, there is still behavioral evidence of learning, as seen with movement times and path lengths decreasing and the distributions of reach deviations shifting towards the correct movement direction.
